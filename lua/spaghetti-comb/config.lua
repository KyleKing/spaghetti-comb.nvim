...
function M.validate(config)
    ...
    if config.display then
        if
            config.display.max_items and (type(config.display.max_items) ~= "number" or config.display.max_items < 1)
        then
    ...
    if config.history then
        if
            ...
            and (type(config.history.max_entries) ~= "number" or config.history.max_entries < 1)
        then
            table.insert(errors, "history.max_entries must be a positive number")
        end
        if
            config.history.max_age_minutes
            and (type(config.history.max_age_minutes) ~= "number" or config.history.max_age_minutes < 1)
        then
    ...
    if config.visual then
        if
            config.visual.floating_window_width
            and (type(config.visual.floating_window_width) ~= "number" or config.visual.floating_window_width < 10)
        then
            table.insert(errors, "visual.floating_window_width must be at least 10")
        end
        if
            config.visual.floating_window_height
            and (type(config.visual.floating_window_height) ~= "number" or config.visual.floating_window_height < 5)
        then
...
function M.merge(base, override) return vim.tbl_deep_extend("force", base, override or {}) end
