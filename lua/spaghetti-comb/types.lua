...
-- Generate unique ID
function M.generate_id() return string.format("%d_%d", os.time(), math.random(1000, 9999)) end

-- Validate NavigationEntry
function M.validate_navigation_entry(entry)
    if type(entry) ~= "table" then return false, "Entry must be a table" end

    if not entry.id or type(entry.id) ~= "string" then return false, "Entry must have a string id" end

    if not entry.file_path or type(entry.file_path) ~= "string" then
        return false, "Entry must have a string file_path"
    end

    if not entry.position or type(entry.position) ~= "table" then return false, "Entry must have a position table" end
...
function M.validate_navigation_trail(trail)
    if type(trail) ~= "table" then return false, "Trail must be a table" end

    if not trail.entries or type(trail.entries) ~= "table" then return false, "Trail must have an entries array" end
...
function M.validate_bookmark_entry(entry)
    if type(entry) ~= "table" then return false, "Bookmark entry must be a table" end

    if not entry.id or type(entry.id) ~= "string" then return false, "Bookmark entry must have a string id" end
