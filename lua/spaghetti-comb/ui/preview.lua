...
function M.get_code_context(location, context_lines)
  ...
  if preview_state.preview_cache[cache_key] then
    return preview_state.preview_cache[cache_key]
  end
  
  if not vim.fn.filereadable(location.path) then
    return nil
  end
...
function M.format_preview_lines(context)
  if not context or not context.lines then
    return {}
  end
...
function M.is_item_expanded(item_key)
  return preview_state.expanded_items[item_key] or false
end
...
function M.expand_item(item_key)
  preview_state.expanded_items[item_key] = true
end

function M.collapse_item(item_key)
  preview_state.expanded_items[item_key] = false
end

function M.clear_expanded_items()
  preview_state.expanded_items = {}
end

function M.clear_preview_cache()
  preview_state.preview_cache = {}
end
...
function M.create_preview_content(location, context_lines)
  ...
  table.insert(preview_lines, 1, string.format("[Preview: %s]", 
    utils.get_relative_path(location.path) or location.path))
