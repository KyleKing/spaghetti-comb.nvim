...
local function get_coupling_highlight(coupling_score)
  if not coupling_score then
    return 'SpaghettiCombCoupling'
  end
...
function M.apply_highlights(buf_id)
  ...
  for line_num, line in ipairs(lines) do
    ...
    elseif line:match("^References") or line:match("^Definitions") or 
           line:match("^Incoming Calls") or line:match("^Outgoing Calls") then
    ...
    elseif line:match("^[├└]─") then
      ...
      if colon_pos then
        ...
        if bracket_start then
          vim.api.nvim_buf_add_highlight(buf_id, ns_id, 'SpaghettiCombLocation', line_idx, colon_pos - 1, bracket_start - 1)
